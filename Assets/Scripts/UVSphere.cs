...
public class UVSphere : MonoBehaviour {
    ...
    [Range(min: 1, max: 256)]
    public float width ; // width de la esfera (X)
    [Range(min: 1, max: 256)]
    public float length ; // length de la esfera (Z)
    [Range(min: 1, max: 256)]
    public float high ; // high de la esfera (Y)

    public bool showVertices;

   [Range(min:2,max:256)]
    public int resolution = 8; // Es el numero de subdivisiones de la esfera.
